...
const Modal: React.FC<ModalProps> = ({
  ...
  /**
   * 💡 CORRECTION : On retire "if (disabled) return"
   * On veut pouvoir fermer la modale même si le formulaire n'est pas valide.
   */
  const handleClose = useCallback(() => {
    setShowModal(false)

    // On attend la fin de l'animation avant de démonter le composant
    setTimeout(() => {
      onClose()
    }, 300)
  }, [onClose])
        ...
        >
          <div className="h-full lg:h-auto md:h-auto border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div className="flex items-center p-6 rounded-t justify-center relative border-b">
              {/* La croix de fermeture appelle handleClose qui n'est plus bloqué par "disabled" */}
              <button
                onClick={handleClose}
            ...
            <div className="flex flex-col gap-2 p-6">
              <div className="flex flex-row items-center gap-4 w-full">
                ...
                <CustomButton
                  disabled={disabled} // Le bouton de validation reste bloqué si nécessaire
                  label={actionLabel}
